...
boolean inslivello(char *elem, ABR T, int n, boolean impiegato)
{
  ...
  instabpos(&T->livelli, n, impiegato);
...
ABR costruisceABR (char *elem, ABR Ts, ABR Td)
{
  ...
  strcpy(T->val, elem);
  creatablivelli(&T->livelli, NUMERO_LIVELLI);
  T->padre = NULL;
  T->Ts = Ts;
  if (Ts != NULL)
    Ts->padre = T;
...
void stampaABRLivelli(ABR T)
{
  ...
  if (T != NO_TREE)
  {
    stampaABRLivelli(T->Ts);
    if( (T->livelli.LI[1] != 0) || (T->livelli.LP[1] != 0) )
      printf("%s \n", T->val);
    stampatablivelli2(&((nodoABR(T->val, T))->livelli));
    stampaABRLivelli(T->Td);
  }
}